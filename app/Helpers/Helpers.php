...
function getProductsByCategory($category)
{
    ...
    switch ($category) {
        ...
        default:
            ...
            $products = Product::whereIn('id', $product_ids)->where('status', '=', 'published')->where('approved', 1)->orderby('id', 'DESC')->take(15)->get();
    }
...
function globalproductsonesignal($id)
{
    $product = Product::where('id', $id)->get();
    foreach ($product as $value) {
        $value->specification;
        foreach ($value->images as $image) {
            $img[] = [
                'mediumUrl' => $value->getImageAttribute()->mediumUrl,
                'relativePath' => 'asd',
                'url' => 'as',
                'smallUrl' => 'as',
                'largeUrl' => 'as',
                'largeslideshowUrl' => 'as',
