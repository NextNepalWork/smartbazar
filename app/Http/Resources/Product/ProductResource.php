...
class ProductResource extends Resource
{
    ...
    public function toArray($request)
    {
        ...
        foreach ($similarProducts as $similarProductss) {
//            $similarProduct->img = $similarProduct->getImageAttribute()->mediumUrl;
            $similarProduct[] = globalproducts($similarProductss->id);
        }
        ...
        $i = 0;
        foreach ($related_products as $related_productss) {
            $relatedProducts[] = globalproducts($related_productss->id);
...
//            $relatedProducts[$i]['image'] = $related_product->getImageAttribute()->smallUrl;
        ...
        return [
            ...
            'similarProducts' =>isset($similarProduct)? $similarProduct :[],
            'wishlist' => isset($wishlist) ? 1 : 0,
            'related_products' => isset($relatedProducts)?$relatedProducts :''
        ];
    }
}
