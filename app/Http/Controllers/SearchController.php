...
class SearchController extends Controller {
    ...
    public function getSearch(Request $request ) {
        ...
        // else {
            ...
            $products = Product::where('approved',$approved=1)->where( 'status',$status='published' )->whereHas( 'categories', function ( $query ) use ( $queryParam ){
            ...
            if($request->has('brand') ||  $request->has('size') || $request->has('colour') ||  $request->has('maxprice') || $request->has('minprice') ||$request->has('sort') ){
                ...
                if($request->has('sort') ){
                    ...
                    if(isset($sortBy)){
                        ...
                        if ($sortBy == 'discount') {
                        $products = $products()->orderBy ('product_price' - 'sale_price');
