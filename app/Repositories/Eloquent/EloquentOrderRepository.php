...
class EloquentOrderRepository extends AbstractRepository implements OrderRepository
{
    ...
    public function store(array $attributes)
    {
        ...
        $address = ShippingAccount::updateOrCreate(['id' => $attributes['address_id'], 'user_id' => auth()->id()], $addressData);
        $attributes['address_id'] = $address->id;
        $location = $attributes['shipping_address'];
    ...
    {
        $number = mt_rand(0000000000, mt_getrandmax());
    ...
    public function update($id, array $attributes)
    {
        ...
        if (Cart::instance('prebooking')->count() > 0) {
            ...
            if ($attributes['order_status'] == 5 && $order->order_status_id != 1 && $order->order_status_id != 5) {
                foreach ($attributes['products'] as $orderProduct) {
                    $pid = $orderProduct['id'];
                    $qty = $orderProduct['qty'];
                    $product = Product::where('id', $pid)->first();
            ...
            $order->update([
                ...
                'delivery_destination_id' => $attributes['delivery_destination'],
