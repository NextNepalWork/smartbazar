...
function addToMenu(param) {
    $("#spincustomu").show();

    var data = [];

    $($(param).parent().parent().find('ul li')).each(function (i, row) {

        var labelmenu = $(row).find('input:checked').attr('data-label');
        var linkmenu = $(row).find('input:checked').val();
        // var idmenu = $("#idmenu").val();

        if (labelmenu && linkmenu ) {
            data.push({
                labelmenu: labelmenu,
                linkmenu: linkmenu,
                // idmenu: idmenu
