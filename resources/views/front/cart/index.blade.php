                        ...
                        @foreach(Cart::instance('default')->content() as $cartContent)
                            <div class="product cart_item">
                                ...
                                <div class="product-unitprice"
                                     style="display: none;">{{ \App\Model\Product::where('id',$cartContent->id)->first()->tax }}</div>
                                <div class="product-quantity product-quantitydb">
                                    <input type="number" class="form-control" data-product="{{$cartContent->rowId}}"
                                           value="{{ $cartContent->qty }}" min="1"
                                           max="{{\App\Model\Product::where('id',$cartContent->id)->first()->stock_quantity}}">
                                </div>
                                <div class="product-removal product-remove">
                                    <button class=" btn btn-danger remove-product btn-remove-row remove"
                                            data-row="{{ $cartContent->rowId }}">
                                        <i class="fa fa-trash"></i>
                                    </button>
                                </div>
                                <div class="product-line-price product-subtotal">{{ $cartContent->qty * $cartContent->price }}</div>
                            </div>
                            @php
                        ...
                        <div class="coupon">
                            <div class="float-right">
                                <form action="{{ route('coupon.check') }}" method="post">
                                    ...
                                    <input type="submit" class="uk-button view-cart coupon_btn float-right my-2"
                                           name="apply_coupon"
                                           value="Apply Coupon">
                                </form>
                            </div>
                        ...
                        @if(session()->exists('coupon'))
                            <div class="totals-item">
                                ...
                                <div class="totals-value"
                                     id="cart-shipping">{{ number_format(session()->get('coupon')['discount_value'], 2) }}</div>
                            </div>
                        @endif
                        <div class="totals-item totals-item-total">
                            <label>Grand Total</label>
                            <div class="totals-value totals_value"
                                 id="grandtotal">{{ session()->exists('coupon') ? number_format($grandTotal - session()->get('coupon')['discount_value'], 2) : number_format($grandTotal, 2) }}</div>
                        </div>
                    </div>

                    <a class="btn btn-default continue" href="{{ route('home.index') }}"> <span
                                uk-icon="icon:chevron-left"></span>Continue Shopping</a>
