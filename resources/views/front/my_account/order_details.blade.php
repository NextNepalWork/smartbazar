    ...
    <div class="container">
        @if($order)
            @if($order->user_id ==auth()->id())
                <div class="row">
                    <div class="content-box content-box--shadow mx-auto py-md-5 py-3">
                        <div class="panel panel-default no-border-shadow">
                            <div id="ar-right-1">
                                <div class="panel-body">
                                    <h1 class="font-weight-bold mb-3 d-inline-block mr-2">Order Tracking Id:</h1><span class="text-danger">{{ $order->code }}</span>
                                        ...
                                        <table class="table table-bordered table-hover">
                                            ...
                                            @foreach($products = $order->orderProduct as $product)
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <a href="{{route('user.disputes', \App\Model\OrderProduct::where('order_id',$order->id)->where('product_id',$product->products->id)->first()->id)}}" class="btn btn-warning btn-sm">
                                                            Report</a>
                                                        <a href="{{route('order.return', \App\Model\OrderProduct::where('order_id',$order->id)->where('product_id',$product->products->id)->first()->id)}}" class="btn btn-info btn-sm">Return Product</a>
                                                    </td>
                                                </tr>
                                            @endforeach
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-left">{{ $order->payment->name }}</td>
