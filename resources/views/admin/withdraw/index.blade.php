                        ...
                        <tbody>
                            @foreach ($withdraws as $withdraw)
                                <div style="display: none;">
                                    {{ $status = \App\Model\WithdrawStatus::where('id', $withdraw->status)->first()->status }}
                                </div>
                                <tr>
                                    <td>{{ $loop->iteration }}</td>

                                    <td>{{ isset($withdraw->vendors->name) ? $withdraw->vendors->name : '###' }}</td>
                                    <td>{{ $withdraw->amount }}</td>
                                    <td>{{ isset(\App\User::where('id', $withdraw->vendor_id)->first()->phone) ? \App\User::where('id', $withdraw->vendor_id)->first()->phone : '' }}
                                    </td>
                                    <td>{{ $withdraw->method }}</td>

                                    <td>
                                        @if ($withdraw->approve == 1)
                                            <button class="btn btn-primary btn-xs withdraw-verify"
                                                data-id="{{ $withdraw->id }}">Verify</button>
                                        @else
                                            <button class="btn btn-danger btn-xs withdraw-verify"
                                                data-id="{{ $withdraw->id }}">Non-Verify</button>
                                        @endif
                                    </td>
                                    <td><span
                                            class="label label-{{ $status == 'paid' ? 'success' : 'danger' }}">{{ ucfirst($status) }}</span>
                                    </td>
                                    <td>{{ $withdraw->created_at->diffForHumans() }}</td>
                                    <td>
                                        <div style="display: flex;justify-content: space-between;align-items: center;">
                                            <a href="javascript:void(0);" data-id="{{ $withdraw->id }}"
                                                class="btn btn-xs btn-info btn-edit"><i class="fas fa-check"></i> View
                                                Details</a>
                                            <form action="{{ route('admin.withdraw.cancel', $withdraw->id) }}"
                                                id="cancel-withdraw-{{ $withdraw->id }}" method="get">
                                                {{ csrf_field() }}
                                            </form>
                                            <a href="" onclick="
