                ...
                <div class="form-group{{ $errors->has('order_status') ? ' has-error' : '' }}">
                    ...
                    <select name="order_status" class="form-control select2">
                        ...
                        @foreach($orderStatuses as $orderStatus)
                            <option value="{{ $orderStatus->id }}" @if(!empty($order)) @if($orderStatus->id == $order->order_status_id) selected @endif @endif @if(Auth::User()->hasRole('editor') && $orderStatus->id != 6 && $orderStatus->id != 1) disabled @endif>{{ $orderStatus->name }}</option>
                        @endforeach
                    </select>
