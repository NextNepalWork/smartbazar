<div class="tab-pane active" id="general">
    ...
    <div class="form-group{{ $errors->has('site_logo') ? ' has-error' : '' }}">
        ...
        <label for="site_logo" class="col-sm-2 control-label">Email Logo</label>
        <div class="col-sm-10">
            <input type="file" name="email_logo" id="email_logo" class="form-control">
            @if ($errors->has('email_logo'))
                <span class="help-block">
                    {{ $errors->first('email_logo') }}
                </span>
            @endif

            @if(getConfiguration('email_logo'))
                <div class="mt-15 half-width" style="width: 300px;">
                    <img src="{{ url('storage') . '/' . getConfiguration('email_logo') }}"
                         class="thumbnail img-responsive">
                </div>
            @endif
        </div>
