			...
			<table class="table table-horizontal table-striped table-hover table-condensed" id="advertiseTable">
				...
				<tbody>
					...
					<tr>
						...
						<td width="150px">
							@if($ads->image)
							<a href="{{asset('storage').'/'.$ads->image}}" class="product-img-zoom">	<img src="{{ asset('storage').'/'.$ads->image }}" alt="{{ $ads->title }}" style="width: 50px;height: auto"/></a>
					...
					<td>
						<a href="javascript::void(0);" data-id="{{ $ads->id }}" class="btn btn-xs btn-info btn-edit"><i class="fas fa-check"></i> View Details</a>
...
<script>
	...
	$(document).on("click", ".btn-edit", function (e) {
		...
		var viewDetails = "{{ route('admin.ads.edit', ':id') }}";                               
