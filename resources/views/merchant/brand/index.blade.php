	...
	$(document).ready(function(){
		$('#myTable').DataTable({
			...
			columns: [
	    		...
	    		{data: 'status',
                    render: function (data, type, row) {
                        return data == 1 ? '<span class="label label-success">Approved</span>' : '<span class="label label-danger">Unapproved</span>';
                    }
                },
		        {
                    data: 'id',
	...
	$(document).on("click", ".btn-add", function (e) {
        e.preventDefault();
            ...
            var formData = new FormData($('#brandSubmit')[0]);
                ...
                // formData.append('document', $('input[type=file]')[0].files[1]);
	...
	$(document).on("click", ".btn-delete", function (e) {
       ...
       if (!confirm('Are you sure you want to delete?')) {
        ...
        var id = $this.attr('data-id');
     	var tempDeleteUrl = "{{ route('vendor.brands.delete', ':id') }}";                               tempDeleteUrl = tempDeleteUrl.replace(':id', id);
...
<script>
	...
	$(document).on("click", ".btn-edit", function (e) {
        e.preventDefault();
          var $this = $(this);
        var id = $this.attr('data-id');
     	var tempEditUrl = "{{ route('vendor.brands.edit', ':id') }}";
...
<script>
	$(document).on("click", ".btn-update", function (e) {
        ...
        var params   = $('#update').serializeArray();
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('vendor.brands.update') }}",
                contentType: false,
                processData: false,
                cache: false,
                data: formData,
