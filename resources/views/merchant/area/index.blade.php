			...
			<table class="table table-horizontal table-striped table-hover table-condensed" id="areaTable">
				...
				<tbody>
					...
					<tr>
						...
						<td>{{ $area->area }}</td>
						<td>
							<a href="{{ route('vendor.area.edit',$area->id) }}" class="btn btn-xs btn-default"><span class="lnr lnr-pencil"></span></a>
							<form id="delete-form-{{ $area->id }}" action="{{ route('vendor.area.destroy',$area->id) }}" method="post" style="display: none;">
								{{ csrf_field() }}
								{{ method_field('DELETE') }}
							</form>
							<a href="" onclick="
