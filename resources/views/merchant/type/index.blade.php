			...
			<table class="table table-horizontal table-striped table-hover table-condensed" id="typeTable">
				...
				<tbody>
					...
					<tr>
						...
						<td>{{ $type->size }}</td>
						<td>
							<a href="{{ route('vendor.type.edit',$type->id) }}" class="btn btn-xs btn-default"><span class="lnr lnr-pencil"></span></a>
							<form id="delete-form-{{ $type->id }}" action="{{ route('vendor.type.destroy',$type->id) }}" method="post" style="display: none;">
								{{ csrf_field() }}
								{{ method_field('DELETE') }}
							</form>
							<a href="" onclick="
