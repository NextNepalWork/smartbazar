			...
			<table class="table table-horizontal table-striped table-hover table-condensed" id="packageTable">
				...
				<tbody>
					...
					<tr>
						...
						<td>
							<a href="{{ route('vendor.package.edit',$package->id) }}" class="btn btn-xs btn-default"><span class="lnr lnr-pencil"></span></a>
							<form id="delete-form-{{ $package->id }}" action="{{ route('vendor.package.destroy',$package->id) }}" method="post" style="display: none;">
								{{ csrf_field() }}
								{{ method_field('DELETE') }}
							</form>
							<a href="" onclick="
