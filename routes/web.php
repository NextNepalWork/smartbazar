...
Route::any('/storereferral_link/{token?}', 'Vendor\VendorController@referral_link_click')->name('referral_link_clickss');
Route::any('/product_link/{token?}', 'Frontend\ShippingAccountController@referral_link_click')->name('product_link_clicks');
...
Route::get('/coupon/check/ajax', 'HomeController@couponCheckAjax')->name('coupon.check.ajax');
...
Route::get('sell-with-us', 'Front\HomeController@sellWithUs')->name('sell.index')->middleware('auth');
Route::get('sell-service', 'Front\HomeController@sellService')->name('sell.service')->middleware('auth');

Route::get('/', 'Front\HomeController@become_a_seller')->name('sell.become');
Route::post('sell/post', 'Vendor\VendorController@sendVendorRequest')->name('sell.post')->middleware('auth');
Route::get('sell/payment/{data}', 'Vendor\VendorController@vendor_signup_payment')->name('sell.pay')->middleware('auth');
Route::get('sell/pay/{pay_method}/{vendor_code?}', 'Vendor\VendorController@pay_vendor')->name('sell.method')->middleware('auth');
Route::get('fail', 'Vendor\VendorController@fail')->name('sell.fail');
...
Route::group(['namespace' => 'Frontend',], function () {
    ...
    Route::get('/ho', 'HomeController@index')->name('home.index');
    ...
    Route::get('/service/{slug}', 'ServiceController@showCategory')->name('service.link');

    Route::get('/services/{slug}', 'ServiceController@getService')->name('service.show');
...
Route::get('vendor-profile/{name}', 'Front\MallController@vendor_profile')->name('vendor-profile');
Route::get('our-vendors', 'Front\MallController@our_vendors')->name('our-vendors');
Route::get('category-collection', 'Frontend\CategoryController@category_collection')->name('category-collection');
...
Route::group(['middleware' => 'auth', 'namespace' => 'Frontend'], function () {
    ...
    Route::any('user/referral', 'ShippingAccountController@referral')->name('user.referral')->middleware(['role:Super-Customer']);
    ...
    Route::post('/negotiable-create', 'NegotiableController@create')->name('negotiable.create');
...
Route::group(['middleware' => 'auth'], function () {
    Route::group(['prefix' => 'admin', 'as' => 'admin.', 'namespace' => 'Backend', 'middleware' => 'role:admin|manager|editor|delivery'], function () {
        ...
        Route::get('/service/json', 'ServiceController@getServiceJson')->name('services.json');
        ...
        Route::post('/services/time/delete', 'ServiceController@deleteTime')->name('service.time.delete');
        Route::post('/services/location/delete', 'ServiceController@deleteLocation')->name('service.location.delete');


        Route::get('/serviceprovider', 'ServiceProviderController@index')->name('provider.index');
        Route::get('/provider/json', 'ServiceProviderController@getProviderJson')->name('provider.json');
        Route::get('/provider/stats/{id}', 'ServiceProviderController@prodiverStats')->name('provider.stat');
        Route::get('/provider/index/', 'ServiceProviderController@prodiverStatsIndex')->name('provider.stat.index');
        ...
        Route::get('/service-category/delete/{id}', 'ServiceCategoryController@destroy')->name('service.category.delete');
        ...
        Route::any('vendor/commission', 'VendorRequestController@set_commission')->name('vendor.commission');
        Route::any('vendor/referrals', 'VendorRequestController@referrals')->name('vendor.referrals');
        Route::post('vendor/global_commissions', 'VendorRequestController@set_global_commission')->name('vendor.global-commission');
        ...
        Route::get('/orders/export-excel', 'OrderController@exportToExcel')->name('export.excel');
        Route::get('/orders/export-excel/selected', 'OrderController@exportSelectedToExcel')->name('export.selected');
        ...
        //Shipping Amount
        Route::get('/shipping_amount', 'ShippingAmountController@index')->name('shipping_amount');
        Route::get('/shipping_amount/create', 'ShippingAmountController@create')->name('shipping_amount.create');
        Route::post('/shipping_amount/store', 'ShippingAmountController@store')->name('shipping_amount.store');
        Route::get('/shipping_amount/edit/{id}', 'ShippingAmountController@edit')->name('shipping_amount.edit');
        Route::post('/shipping_amount/update', 'ShippingAmountController@update')->name('shipping_amount.update');
        Route::get('/shipping_amount/delete/{id}', 'ShippingAmountController@destroy')->name('shipping_amount.destroy');
        Route::get('/shipping_amount/json', 'ShippingAmountController@getShippingAmountJson')->name('shipping_amount.json');
    ...
    Route::group(['prefix' => 'admin', 'as' => 'admin.', 'namespace' => 'Backend', 'middleware' => 'role:vendor|admin|Super-Vendor|Regional-Manager'], function () {
    ...
    Route::group(['prefix' => 'vendors', 'as' => 'vendor.', 'namespace' => 'Vendor', 'middleware' => 'role:vendor|Super-Vendor|Regional-Manager'], function () {
        // Dashboard
        Route::get('/', 'VendorController@getDashboard')->name('dashboard');
        Route::any('/add_vendor', 'VendorController@add_vendor')->name('add-vendor')->middleware(['role:Super-Vendor|Regional-Manager']);
        Route::any('all-referral', 'VendorController@all_referral')->name('all-referral')->middleware(['role:Super-Vendor|Regional-Manager']);
        Route::any('referral_link', 'VendorController@referral_link')->name('referral_link')->middleware(['role:Super-Vendor|Regional-Manager']);
        Route::any('wallet', 'VendorController@wallet')->name('wallet')->middleware(['role:Super-Vendor|Regional-Manager']);
        ...
        Route::POST('/vendor/configuration/', 'VendorController@company_image')->name('vendors_details.company-image');
        Route::POST('/vendor/configuration/cover', 'VendorController@cover_image')->name('vendors_details.cover-image');
...
Route::get('{slug}', ['uses' => 'PageController@getPage'])->where('slug', '([A-Za-z0-9\-\/]+)')->name('profile');
